"""
Based off of <vertex> created and distributed by faculty.
...
class Vertex:
    ...
    def __str__(self):
        ...
        :return: The string
        """
        return str(self.id) + ' connectedTo: ' + str([str(x.id) + ':' + str(
            self.connectedTo[x]) for x in  self.connectedTo])
    ...
    def getConnections(self):
        ...
        """
        #print("****" + str(self.connectedTo.keys()))
...
def testVertex():
    ...
    print("A's neighbors ():",
          [vertex.id for vertex in vertexA.getConnections()])
    print(vertexB.id[0] )
